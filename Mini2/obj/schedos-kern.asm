...
schedule(void)
{
  ...
  1000c9:	05 24 70 10 00       	add    $0x107024,%eax
  1000ce:	50                   	push   %eax
...
interrupt(registers_t *reg)
{
  ...
  1000f6:	a1 1c 7a 10 00       	mov    0x107a1c,%eax
...
interrupt(registers_t *reg)
{
  ...
  100134:	a1 1c 7a 10 00       	mov    0x107a1c,%eax
  ...
  10014e:	ff 35 1c 7a 10 00    	pushl  0x107a1c
...
start(void)
{
  10016f:	53                   	push   %ebx
	...
	for (i = 0; i < NPROCS; i++) {
		proc_array[i].p_pid = i;
		proc_array[i].p_state = P_EMPTY;
  100170:	bb 74 70 10 00       	mov    $0x107074,%ebx
start(void)
{
	int i;
  ...
  100193:	68 24 70 10 00       	push   $0x107024
  ...
  10019d:	83 c4 10             	add    $0x10,%esp
	...
	for (i = 0; i < NPROCS; i++) {
		proc_array[i].p_pid = i;
  1001a0:	c7 05 24 70 10 00 00 	movl   $0x0,0x107024
  1001a7:	00 00 00 
		proc_array[i].p_state = P_EMPTY;
  1001aa:	c7 05 6c 70 10 00 00 	movl   $0x0,0x10706c
  1001b1:	00 00 00 
	console_clear();
	...
	for (i = 0; i < NPROCS; i++) {
		proc_array[i].p_pid = i;
  1001b4:	c7 05 74 70 10 00 01 	movl   $0x1,0x107074
  1001bb:	00 00 00 
		proc_array[i].p_state = P_EMPTY;
  1001be:	c7 05 bc 70 10 00 00 	movl   $0x0,0x1070bc
  1001c5:	00 00 00 
	console_clear();
	...
	for (i = 0; i < NPROCS; i++) {
		proc_array[i].p_pid = i;
  1001c8:	c7 05 c4 70 10 00 02 	movl   $0x2,0x1070c4
  1001cf:	00 00 00 
		proc_array[i].p_state = P_EMPTY;
  1001d2:	c7 05 0c 71 10 00 00 	movl   $0x0,0x10710c
  1001d9:	00 00 00 
	console_clear();
	...
	for (i = 0; i < NPROCS; i++) {
		proc_array[i].p_pid = i;
  1001dc:	c7 05 14 71 10 00 03 	movl   $0x3,0x107114
  1001e3:	00 00 00 
		proc_array[i].p_state = P_EMPTY;
  1001e6:	c7 05 5c 71 10 00 00 	movl   $0x0,0x10715c
  1001ed:	00 00 00 
	console_clear();
	...
	for (i = 0; i < NPROCS; i++) {
		proc_array[i].p_pid = i;
  1001f0:	c7 05 64 71 10 00 04 	movl   $0x4,0x107164
  1001f7:	00 00 00 
		proc_array[i].p_state = P_EMPTY;
  1001fa:	c7 05 ac 71 10 00 00 	movl   $0x0,0x1071ac
  100201:	00 00 00 
	for (i = 1; i < NPROCS; i++) {
  ...
  100238:	68 74 70 10 00       	push   $0x107074
		proc->p_state = P_RUNNABLE;
	}
  ...
  100244:	80 0b 00 
	...
	// Initialize the scheduling algorithm.
	scheduling_algorithm = 0;
  100247:	c7 05 20 7a 10 00 00 	movl   $0x0,0x107a20
  10024e:	00 00 00 
...
segments_init(void)
{
  ...
  100258:	b8 b4 71 10 00       	mov    $0x1071b4,%eax
...
segments_init(void)
{
  ...
  100279:	88 15 3c 10 10 00    	mov    %dl,0x10103c
	...
	for (i = 0; i < sizeof(interrupt_descriptors) / sizeof(gatedescriptor_t); i++)
		SETGATE(interrupt_descriptors[i], 0,
  10027f:	ba 1c 72 10 00       	mov    $0x10721c,%edx
segments_init(void)
{
	int i;
...
segments_init(void)
{
  ...
  1002a2:	c7 05 b8 71 10 00 00 	movl   $0x180000,0x1071b8
  1002a9:	00 18 00 
	kernel_task_descriptor.ts_ss0 = SEGSEL_KERN_DATA;
  1002ac:	66 c7 05 bc 71 10 00 	movw   $0x10,0x1071bc
  1002b3:	10 00 
	...
	for (i = 0; i < sizeof(interrupt_descriptors) / sizeof(gatedescriptor_t); i++)
		SETGATE(interrupt_descriptors[i], 0,
  1002b5:	66 89 0c c5 1c 72 10 	mov    %cx,0x10721c(,%eax,8)
  1002bc:	00 
  ...
  1002db:	b8 1a 00 10 00       	mov    $0x10001a,%eax
	...
	for (i = INT_SYS_YIELD; i < INT_SYS_YIELD + 10; i++)
		SETGATE(interrupt_descriptors[i], 0,
  1002e0:	ba 1c 72 10 00       	mov    $0x10721c,%edx
	for (i = 0; i < sizeof(interrupt_descriptors) / sizeof(gatedescriptor_t); i++)
		SETGATE(interrupt_descriptors[i], 0,
			SEGSEL_KERN_CODE, default_int_handler, 0);
	...
	// The clock interrupt gets special handling
	SETGATE(interrupt_descriptors[INT_CLOCK], 0,
  1002e5:	66 a3 1c 73 10 00    	mov    %ax,0x10731c
  1002eb:	c1 e8 10             	shr    $0x10,%eax
  1002ee:	66 a3 22 73 10 00    	mov    %ax,0x107322
  1002f4:	b8 30 00 00 00       	mov    $0x30,%eax
  1002f9:	66 c7 05 1e 73 10 00 	movw   $0x8,0x10731e
  100300:	08 00 
  100302:	c6 05 20 73 10 00 00 	movb   $0x0,0x107320
  100309:	c6 05 21 73 10 00 8e 	movb   $0x8e,0x107321
  ...
  10031e:	66 89 0c c5 1c 72 10 	mov    %cx,0x10721c(,%eax,8)
  100325:	00 
...
run(process_t *proc)
{
  100479:	8b 44 24 04          	mov    0x4(%esp),%eax
	current = proc;
  10047d:	a3 1c 7a 10 00       	mov    %eax,0x107a1c
